  ...
  testData(): Element[]{
    ...
    this.apps.bestAndWorstUsersData().subscribe(
        ...
        this.data1 = [
            ...
            { position: 2, name: 'Helium', weight: 4.0026, symbol: 'He', last:'B' },
            { position: 3, name: 'Lithium', weight: 6.941, symbol: 'Li', last:'C' },
            { position: 4, name: 'Beryllium', weight: 9.0122, symbol: 'Be' , last:'D'},
            { position: 5, name: 'Boron', weight: 10.811, symbol: 'B', last:'E' }
        ];

        this.data1 = new ExampleDataSource();
            const rows = [];
            console.log(this.data1);
            this.data1.forEach(element => rows.push(element, { detailRow: true, element }));
            console.log(rows);
...
export class appDataSource extends DataSource<any> {
    constructor(private userService: BlueprismService) {
      super();
    }
    connect(): Observable<Application[]> {
      return this.userService.bestAndWorstUsersData();
    }
    disconnect() {}
  }
