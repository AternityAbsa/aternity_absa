    ...
    loadDoughnut(){
        ...
        this.blueprismService.deviceInventoryData().subscribe(
            ...
            this.dataModel.MICROSOFT_STABILITY_INDEX = blue_applications['value'].map(blue_applications => blue_applications.MICROSOFT_STABILITY_INDEX);
               this.doughnut_time = blue_applications['value'].map(blue_applications => blue_applications.last 

            /** Remove Duplicates **/
            this.deviceName = new Set (this.device_name); 
            this.time_Frame = new Set(this.timeframe);

             this.doughnutChartData = [this.dataModel.DEVICE_CPU_GENERATION, this.dataModel.MICROSOFT_STABILITY_INDEX, 88, 20]; 
