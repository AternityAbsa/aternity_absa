...
export class ChartsComponent implements OnInit {
    ...
    loadDoughnut(){
        ...
        this.blueprismService.deviceInventoryData().subscribe(
            ...
            /** Remove Duplicates **/
            this.doughnut_time =  this.dataModel.DEVICE_CPU_CORES; 
            console.log(this.doughnut_time);
            this.doughnutChartData = this.doughnut_time;  
        },
        err => {
          console.log(err);
        }
    ...
    loadRadar(){
        this.blueprismService.applicationRawData().subscribe(
            ...
            this.radarChartData = [
            /*{ data: this.radar_uxi, label: 'UXI' },   
            { data: this.radar_crashes, label: 'CRASHES' } **/

            { data: [65, 59, 90, 81, 56, 55, 40, 50], label: 'UXI' },
            { data: [28, 48, 40, 19, 96, 27, 100, 80], label: 'UXI_WEIGHT' }
